...
#model = load_model('best_model.h5',compile=False,custom_objects={'LayerNormalization':LayerNormalization,'OurBidirectional':OurBidirectional})
    ...
    for i in range(maxlen): # 强制要求输出不超过maxlen字
        ...
        if len(ends) > 0:
           ...
           #    if id2str(yid[t+1]) != id2str(yid[t])
            ...
            return clean_punct(id2str(yid[0])), clean_punct(id2str(yid[1])),clean_punct(id2str(yid[k]))
    # 如果maxlen字都找不到<end>，直接返回
    return clean_punct(id2str(yid[np.argmax(scores)])) * 3
